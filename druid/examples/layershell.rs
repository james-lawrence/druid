...
use druid::{AppLauncher, Data, Lens, UnitPoint, Widget, WidgetExt};
use druid_shell::platform::wayland;
...
pub fn main() {
    let widget = druid::WindowDesc::new(main_widget()).backend(Box::new(|app, winhandle| {
        wayland::layershell::Builder::new(app.clone())
            .keyboard_interactivity(wayland::layershell::KeyboardInteractivity::None)
            .exclusive_zone(32)
            .anchor(
                wayland::layershell::Anchor::Top
                    | wayland::layershell::Anchor::Left
                    | wayland::layershell::Anchor::Right,
            )
            .size((0.0, 32.0))
            .layer(wayland::layershell::Layer::Top)
            .set_handler(winhandle)
            .build()
    }));
    AppLauncher::with_window(widget)
        .log_to_console()
        .launch(AppData {
            option: true,
